...
if ($priceIndividual !== null && $priceIndividual > $maxPrice) {
    $message = [
        'type' => 'error',
        'text' => 'El precio individual no puede ser mayor a $10,000'
    ];
    $action = 'add';
    $error = true;
}
...
if ($priceWholesale !== null && $priceWholesale > $maxPrice) {
    $message = [
        'type' => 'error',
        'text' => 'El precio mayoreo no puede ser mayor a $10,000'
    ];
    $action = 'add';
    $error = true;
}
...
if ($priceWholesale !== null && $priceWholesale > $maxPrice) {
    ...
    $action = 'add';
        ...
        // sizes come as array of size names from checkboxes
...
if ($action === 'edit' && isset($_GET['id'])) {
    ...
    if ($editProduct) {
        $vstmt = $db->prepare("SELECT size FROM product_variants WHERE product_id = ? ORDER BY FIELD(size,'XS','S','M','L','XL','XXL','XXXL')");
...
$products = $db->query("
    SELECT p.*, c.name as category_name,
        ...
        (SELECT GROUP_CONCAT(size ORDER BY FIELD(size,'XS','S','M','L','XL','XXL','XXXL') SEPARATOR ',') FROM product_variants WHERE product_id = p.id) as sizes
...
<head>
    ...
    <style>
        ...
        /* Price fields dynamic show/hide */
        .price-section { transition:opacity .2s, max-height .3s; }
        ...
        <form method="POST" enctype="multipart/form-data" class="product-form">
            ...
            <div class="form-grid-3" style="margin-bottom:20px">
                ...
                <div class="form-group">
                    ...
                    <select name="sale_type" class="form-input" id="sale_type_select" onchange="togglePrices(this.value)">
            ...
            <div class="form-group" style="margin-bottom:24px">
                ...
                <div class="sizes-grid">
                    ...
                    <label style="cursor:pointer">
                        <input type="checkbox" name="sizes[]" value="<?= $sz ?>"
                            <?= in_array($sz, $editSizes) ? 'checked' : '' ?>
                            style="display:none" class="size-checkbox">
                        <span class="size-btn <?= in_array($sz, $editSizes) ? 'active' : '' ?>"
                ...
                <p style="font-size:.68rem;color:var(--gray);margin-top:10px">
                    ...
                    Selecciona las tallas disponibles. Si no aplica (ej. paquetes), deja vacío.
                </p>
            </div>
...
<script>
// Dynamic price fields based on sale type
function togglePrices(type) {
    const indWrap = document.getElementById('price_individual_wrap');
    const mayWrap = document.getElementById('price_wholesale_wrap');
    const minWrap = document.getElementById('min_wholesale_wrap');
    if (!indWrap) return;
    indWrap.classList.toggle('hidden', type === 'wholesale');
    mayWrap.classList.toggle('hidden', type === 'individual');
    minWrap.classList.toggle('hidden', type === 'individual');
}
// Run on load
(function() {
...
async function logout(e) {
    e.preventDefault();
    const r = await fetch('../api/auth.php', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'logout'})});
