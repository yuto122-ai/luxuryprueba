...
// ===========================
// BASE URL DINÁMICO (detecta automáticamente)
// ===========================

$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? 'https://' : 'http://';
$host = $_SERVER['HTTP_HOST'];

// config.php está en /luxuryprueba/php/
// __DIR__ = /path/to/luxuryprueba/php
// dirname(__DIR__) = /path/to/luxuryprueba
$luxurypruebaDirPath = dirname(__DIR__);

// DOCUMENT_ROOT = /xampp/htdocs/ejemplos/ringo cambios
// Obtener la ruta relativa desde DOCUMENT_ROOT
$docRoot = rtrim(str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']), '/');
$luxurypruebaDirPath = str_replace('\\', '/', $luxurypruebaDirPath);

// Calcular ruta relativa y urlencode para manejar espacios
$relativePath = substr($luxurypruebaDirPath, strlen($docRoot));
if ($relativePath === false || $relativePath === '') {
    $relativePath = '';
} else {
    // Separar por / y urlencode cada segmento para manejar espacios como "ringo cambios"
    $segments = array_filter(explode('/', $relativePath));
    $relativePath = '/' . implode('/', array_map('rawurlencode', $segments));
}
...
// Stripe keys: define in environment or hardcode here for local development
// Reemplaza con tus claves reales de Stripe (modo test / modo producción)
