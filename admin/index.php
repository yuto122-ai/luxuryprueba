...
<div class="admin-layout">
    ...
    <aside class="admin-sidebar">
        ...
        <nav class="admin-nav">
            ...
            <a href="../api/auth.php" onclick="logout(event)"><i class="fas fa-sign-out-alt"></i> Salir</a>
        </nav>
    </aside>
    ...
    <main class="admin-main">
        <div class="admin-header-top">
            <div>
                ...
                <p style="font-size:.72rem;color:var(--gray);margin-top:4px"><?= date('l, d \d\e F Y') ?></p>
            </div>
        ...
        <div class="stat-cards">
            ...
            <div class="stat-card" style="border-bottom:2px solid <?= $pendingOrders > 0 ? '#ffc800' : 'var(--gold)' ?>">
                <div class="stat-value" style="color:<?= $pendingOrders > 0 ? '#ffc800' : 'var(--white)' ?>"><?= $pendingOrders ?></div>
        ...
        <div class="quick-actions">
            <a href="products.php" class="btn btn-dark"><i class="fas fa-tshirt"></i> Gestionar Productos (<?= $totalProducts ?>)</a>
            <a href="orders.php?status=pending" class="btn btn-dark"><i class="fas fa-clock"></i> Ver Pendientes</a>
            <a href="settings.php" class="btn btn-dark"><i class="fab fa-telegram"></i> Config Telegram</a>
        </div>

        <!-- RECENT ORDERS -->
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
            <h2 style="font-family:var(--font-serif);font-size:1.4rem;font-weight:300">Últimos Pedidos</h2>
            <a href="orders.php" style="font-size:.7rem;color:var(--gold);letter-spacing:.2em;text-decoration:none;text-transform:uppercase">
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr><td colspan="8" style="text-align:center;color:var(--gray);padding:40px">No hay pedidos aún</td></tr>
                <?php else: ?>
                <?php foreach($recentOrders as $order): ?>
                <tr>
                    <td><strong style="color:var(--gold)"><?= htmlspecialchars($order['order_number']) ?></strong></td>
                    <td>
                        <div style="font-size:.85rem"><?= htmlspecialchars($order['customer_name'] ?? $order['shipping_name']) ?></div>
                        <div style="font-size:.7rem;color:var(--gray)"><?= htmlspecialchars($order['shipping_phone']) ?></div>
                    </td>
                    <td>
                        <span style="font-size:.65rem;letter-spacing:.15em;text-transform:uppercase;color:<?= $order['order_type'] === 'wholesale' ? 'var(--gold)' : 'var(--gray-light)' ?>">
                    ...
                    <td style="text-align:center">
                        <?= $order['telegram_sent'] ? '<span style="color:#4acc80"><i class="fas fa-check"></i></span>' : '<span style="color:var(--gray)"><i class="fas fa-minus"></i></span>' ?>
                    </td>
                    <td style="font-size:.78rem;color:var(--gray)"><?= date('d/m/Y H:i', strtotime($order['created_at'])) ?></td>
                    <td>
                        <a href="orders.php?view=<?= $order['id'] ?>" class="btn btn-dark" style="font-size:.6rem;padding:6px 14px">
...
async function logout(e) {
    e.preventDefault();
    const res = await fetch('../api/auth.php', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'logout'})});
