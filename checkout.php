...
async function loadCheckoutCart() {
    // Reset del estado del formulario al cargar
    orderType = 'individual';
    isPlacingOrder = false;
    
    // Reset inputs
    document.getElementById('shipping-name').value = '<?= isLoggedIn() ? htmlspecialchars($_SESSION['user_name'] ?? '') : '' ?>';
    document.getElementById('shipping-phone').value = '';
    document.getElementById('shipping-address').value = '';
    document.getElementById('shipping-email').value = '<?= isLoggedIn() ? htmlspecialchars($_SESSION['user_email'] ?? '') : '' ?>';
    document.getElementById('order-notes').value = '';
    
    // Reset UI state
    setCheckoutStep('shipping');
    document.getElementById('type-individual').classList.add('active');
    document.getElementById('type-wholesale').classList.remove('active');
...
async function placeOrder() {
    ...
    try {
        ...
        if (!data.success || !data.id) {
            throw new Error(data.error || data.message || 'No se pudo iniciar checkout con Stripe');
        }
        ...
        document.getElementById('checkout-alert').innerHTML = `<div class="alert alert-error">${e.message}</div>`;
